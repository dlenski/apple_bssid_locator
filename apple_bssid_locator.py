...
def query_bssid(bssid):
	...
	# .to_bytes defaults to network/big-endian byte order starting in Python 3.11
	data = (
		(1).to_bytes(2, 'big') +
		len(locale).to_bytes(2, 'big') + locale +
		len(dotted_class).to_bytes(2, 'big') + dotted_class +
		len(version).to_bytes(2, 'big') + version +
		(1).to_bytes(4, 'big') +
